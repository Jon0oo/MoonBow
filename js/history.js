...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (window.location.pathname.endsWith("history.html")) {
        ...
        let index = 0;
        let refreshCounter = 0; // Tracks cycles
        ...
        function typeWriter() {
            ...
            if (index < text.length) {
                if (text.substring(index, index + 4) === "<br>") {
                    ...
                    let nextChunk = text.substring(index, index + 2);

                    if (nextChunk.includes("<")) {
                        textSpan.innerHTML += text.charAt(index);
                        index++;
                    } else {
                        textSpan.innerHTML += nextChunk;
                        index += 2;
                    }
                }
                ...
                // Refresh ScrollTrigger every 10 cycles for smoother animations
