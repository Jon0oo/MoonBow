...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const car = document.querySelector('.carDiv');const lightTrailElement = document.getElementById("lightTrail");
  ...
  timeline.to(car, {
    duration: 1.1,
    motionPath: {
      ease: CustomEase.create("customCar", "M0,0 C0,0 0.085,0.188 0.101,0.249 0.108,0.272 0.17,0.307 0.179,0.321 0.185,0.332 0.269,0.356 0.299,0.375 0.449,0.472 0.672,0.522 0.809,0.626 0.841,0.64 0.893,0.676 0.902,0.684 0.912,0.694 0.929,0.719 0.936,0.75 0.94,0.771 0.96,0.804 0.965,0.826 0.977,0.882 1,1 1,1 "),
    ...
    onUpdate: function () {
      let progress = this.progress(); // Get the current progress (0 to 1)
      if (progress >= 0.95) {
        gsap.to(car, { opacity: 0, duration: 0.1 }); // Fade out at end
      } else {
        gsap.to(car, { opacity: 1, duration: 0.1 }); // Ensure car is visible when scrolling back up
      }
    }
  }, 0);
